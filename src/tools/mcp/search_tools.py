...
def wiki_search(query: str) -> dict:
    ...
    Useful for retrieving concise encyclopedic background information, historical facts,
    definitions, biographies, scientific concepts, and general knowledge from Wikipedia.
 
    Args:
        query (str): The topic to look up on Wikipedia.
    """
    try:
...
def weather_tool(location: str) -> dict:
    ...
    try:
        ...
        report = {
            ...
            "extras": {
                "sunrise": sunrise, "sunset": sunset, "report_time": report_time,
...
@mcp.tool
def stock_finance_tool(symbol: str) -> dict:
    """
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            return {"stock_finance_tool_error": "Failed to get data"}
        ...
        return {"stock_finance_tool_error": f"No stock data available for '{symbol}': {data}"}